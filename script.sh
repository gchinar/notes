...
./check_oidc_setup.sh \
  working-gitactions-role \
  failing-gitactions-role \
  arn:aws:iam::<AWS_ACCOUNT_ID>:oidc-provider/github.example.com/_services/token

What the script is checking in plain English

It compares:

Working role

The IAM role that already works with GitHub Actions OIDC.

Failing role

The IAM role that fails with the new repo.

OIDC provider

The IAM identity provider used by both roles.

Then it checks:
	•	whether both roles trust the same provider
	•	whether both use the same audience check
	•	whether subject patterns differ
	•	whether the provider itself allows that audience
	•	whether policies attached to the roles differ

Most important output section

This part is usually the most useful:

PROVIDER AUDIENCE CHECK

If you see:

MISMATCH: trust audience '...' NOT found in provider client IDs

that is a strong sign of the root cause.

If you want, I can also make a second small script that compares only the trust relationship JSON and prints just the important aud and sub differences.
